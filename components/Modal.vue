<template>
  <div class="z-6500 fixed inset-0 flex items-center justify-center px-4 py-6 sm:py-12">
    <div class="absolute inset-0 bg-black/60 backdrop-blur" @click="closeModal" aria-hidden="true"></div>
    <div
      :class="[
        'relative w-full max-w-lg rounded-3xl border border-white/15 bg-white/10 px-6 py-6 shadow-glass backdrop-blur-2xl transition sm:px-8 sm:py-8',
        errorStatusClass
      ]"
      role="dialog"
      aria-modal="true"
    ...
    >
      <button
        type="button"
        @click="closeModal"
        class="absolute right-4 top-4 rounded-full border border-white/10 bg-white/10 p-1 text-white/70 transition hover:border-cosmic-aurora/60 hover:text-white focus:outline-none focus:ring-2 focus:ring-cosmic-aurora/70"
        aria-label="Close"
      >
      ...
      <div class="space-y-3 pr-4 text-white">
        <h2 id="modal-title" class="text-xl font-semibold tracking-wide">{{ errorTitle }}</h2>
        <p class="text-sm leading-relaxed text-white/70">{{ errorMessageText }}</p>
      </div>
    </div>
  </div>
</template>
