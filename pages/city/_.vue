...
export default {
  ...
  created() {
    ...
    this.$store.commit('updateQuery', 'city:' + this.query)
  },
  watch: {
  ...
  head() {
    return {
      title: 'City results for ' + this.query,
      meta: [{
        hid: 'description',
        name: 'description',
        content: 'Explore latest city results ' + this.query
      }]
    }
  },
  methods: {
    fetchLatest(query) {
      this.$axios.$get(process.env.API_URL + '/match/city:' + query).then(res => {
        this.results = res
      }).catch((error) => {
        if (error.response) {
