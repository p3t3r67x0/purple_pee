...
export default {
  ...
  created() {
    ...
    this.$store.commit('updateQuery', 'mx:' + this.query)
  },
  computed: {
    modalVisible() {
  ...
  head() {
    return {
      title: 'MX record results for ' + this.query,
      meta: [{
        hid: 'description',
        name: 'description',
        content: 'Explore latest MX record results ' + this.query
      }]
    }
  },
  methods: {
    fetchLatest(query) {
      this.$axios.$get(process.env.API_URL + '/match/mx:' + query).then(res => {
        this.results = res
      }).catch((error) => {
        if (error.response) {
