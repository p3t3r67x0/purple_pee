...
export default {
  ...
  created() {
    ...
    this.$store.commit('updateQuery', 'banner:' + this.query)
  },
  computed: {
    modalVisible() {
  ...
  head() {
    return {
      title: 'SSH banner results for ' + this.query,
      meta: [{
        hid: 'description',
        name: 'description',
        content: 'Explore latest SSH banner results ' + this.query
      }]
    }
  },
  methods: {
    fetchLatest(query) {
      this.$axios.$get(process.env.API_URL + '/match/banner:' + query).then(res => {
        this.results = res
      }).catch((error) => {
        if (error.response) {
