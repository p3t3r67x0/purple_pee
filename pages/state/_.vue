...
export default {
  ...
  created() {
    ...
    this.$store.commit('updateQuery', 'state:' + this.query)
  },
  watch: {
  ...
  head() {
    return {
      title: 'State results for ' + this.query,
      meta: [{
        hid: 'description',
        name: 'description',
        content: 'Explore latest state results ' + this.query
      }]
    }
  },
  methods: {
    fetchLatest(query) {
      this.$axios.$get(process.env.API_URL + '/match/state:' + query).then(res => {
        this.results = res
      }).catch((error) => {
        if (error.response) {
