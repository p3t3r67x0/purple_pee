...
export default {
  ...
  created() {
    ...
    this.$store.commit('updateQuery', 'ns:' + this.query)
  },
  computed: {
    modalVisible() {
  ...
  head() {
    return {
      title: 'NS record results for ' + this.query,
      meta: [{
        hid: 'description',
        name: 'description',
        content: 'Explore latest NS record results ' + this.query
      }]
    }
  },
  methods: {
    fetchLatest(query) {
      this.$axios.$get(process.env.API_URL + '/match/ns:' + query).then(res => {
        this.results = res
      }).catch((error) => {
        if (error.response) {
